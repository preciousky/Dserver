...
var server = http.createServer(function (req, res) {
    ...
    req.on('end', function () {
        ...
        // }
        if (URL=='/user/myBusiness/getUserPapersById') {
            fabricQuery('queryPaper', ['zzzzz2'])
                .then((fabricRes) => {
                    responseData_json = {
                        "cards": [JSON.parse(fabricRes)]
                    }
                    httpRes(JSON.stringify(responseData_json));
                })
                .catch((err) => {
                    console.error("Hyperledger Fabric caught Error", err);
                    httpRes('My FABRIC error!!!');
                });
        }
